...
def test_reports_directory_remains_private(tmp_path: Path) -> None:
    init_repo(tmp_path)
    (tmp_path / "REPORTS").mkdir()
    (tmp_path / "REPORTS" / "REPORT.md").write_text("private\n", encoding="utf-8")
    (tmp_path / "REPORTS" / "notes.md").write_text("private\n", encoding="utf-8")
    git(tmp_path, "add", "-f", "REPORTS/REPORT.md", "REPORTS/notes.md")

    assert find_tracked_prohibited(tmp_path) == [
        "REPORTS/REPORT.md",
        "REPORTS/notes.md",
    ]


def test_root_report_is_public(tmp_path: Path) -> None:
    init_repo(tmp_path)
    (tmp_path / "REPORT.md").write_text("public\n", encoding="utf-8")
    git(tmp_path, "add", "REPORT.md")
